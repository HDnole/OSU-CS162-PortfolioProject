...
class CountSeqTests(unittest.TestCase):
    ...
    def test_7(self):
        """
        test GessGame.resign_game() for black
        """
        gg = GessGame()
        gg.resign_game()
        self.assertEqual('WHITE_WON', gg.get_game_state())

    def test_8(self):
        """
        test GessGame.resign_game() for white
        """
        gg = GessGame()
        self.assertTrue(gg.make_move('m7', 'k7'))
        gg.resign_game()
        self.assertEqual('BLACK_WON', gg.get_game_state())
